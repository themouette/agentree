...
/// - If config.location is None, compute sibling directory: ../worktrees
...
pub fn compute_worktree_path(
    ...
    let base_dir = if let Some(location) = &config.location {
        ...
        // Compute sibling directory: ../worktrees
        let parent = repo_root.parent().unwrap_or(repo_root);
        parent.join("worktrees")
    };
...
mod tests {
    ...
    fn test_expand_date_format() {
        ...
        // Verify it matches YYYY-MM-DD pattern by checking structure
        let chars: Vec<char> = result.chars().collect();
        assert!(chars[0].is_ascii_digit()); // Year digit 1
        assert!(chars[1].is_ascii_digit()); // Year digit 2
        assert!(chars[2].is_ascii_digit()); // Year digit 3
        assert!(chars[3].is_ascii_digit()); // Year digit 4
        assert_eq!(chars[4], '-'); // First dash
        assert!(chars[5].is_ascii_digit()); // Month digit 1
        assert!(chars[6].is_ascii_digit()); // Month digit 2
        assert_eq!(chars[7], '-'); // Second dash
        assert!(chars[8].is_ascii_digit()); // Day digit 1
        assert!(chars[9].is_ascii_digit()); // Day digit 2
    }
    ...
    fn test_default_path_sibling_directory() {
        let config = WorktreeConfig::default(); // No location, template = "{repo}/{branch}"
        ...
        assert_eq!(
            result,
            PathBuf::from("/home/user/worktrees/myproject/feature")
        );
    }
    ...
    fn test_default_template_branch() {
        ...
        let result = compute_worktree_path(&config, &repo_root, &ctx).unwrap();
        // Default template is "{repo}/{branch}", so worktree at ../worktrees/myrepo/hotfix
        assert_eq!(result, PathBuf::from("/home/user/worktrees/myrepo/hotfix"));
    }

    #[test]
    fn test_sibling_directory_uses_repo_name() {
        // The sibling directory should be named "worktrees", not based on repo name
        let config = WorktreeConfig::default();
        ...
        let result = compute_worktree_path(&config, &repo_root, &ctx).unwrap();
        // Sibling directory name: worktrees (not my-awesome-project-worktrees)
        // Template: {repo}/{branch} -> my-awesome-project/feature-new
        assert_eq!(
            result,
            PathBuf::from("/projects/worktrees/my-awesome-project/feature-new")
        );
    }
